...
public class AnimationEventEffects : MonoBehaviour {
    ...
    public void InstantiateEffectHitFX(int EffectNumber)
    {
        Debug.Log("AGag");
        if (HitFXs == null || HitFXs.Length <= EffectNumber)
        {
            Debug.LogError("Incorrect effect number or effect is null");
        }

        var instance = Instantiate(HitFXs[EffectNumber].Effect, HitFXs[EffectNumber].StartPositionRotation.position, HitFXs[EffectNumber].StartPositionRotation.rotation);

        if (HitFXs[EffectNumber].UseLocalPosition)
        {
            Debug.Log("123");
            instance.transform.parent = HitFXs[EffectNumber].StartPositionRotation.transform;
            instance.transform.localPosition = Vector3.zero;
            instance.transform.localRotation = new Quaternion();
        }
        Destroy(instance, HitFXs[EffectNumber].DestroyAfter);
    }
    ...
    void SetTheAnimationPlaySpeed(float value)
    {
        _animator.speed = value;
    }

    void IgoreCollision(string value)
    {
        if (value == "true")
        {
